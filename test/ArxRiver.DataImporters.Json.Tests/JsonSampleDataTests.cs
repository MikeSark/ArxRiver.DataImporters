...
using ArxRiver.DataImporters.Json.Tests.SampleData;

namespace ArxRiver.DataImporters.Json.Tests;

/// <summary>
/// Integration tests using the EmployeeDto with JsonImporter and all validation layers.
/// </summary>
public class JsonSampleDataTests
{
    [Fact]
    public void EmployeeJsonGenerator_CreatesValidJsonFile()
    {
        var path = Path.Combine(Path.GetTempPath(), $"test_{Guid.NewGuid():N}.json");
        try
        {
            EmployeeJsonGenerator.Generate(path);
            Assert.True(File.Exists(path));

            var importer = new JsonImporter<EmployeeDto>(path);
    ...
    [Fact]
    public void EmployeeDto_Json_ValidRows_PassAllValidation()
    {
        var path = Path.Combine(Path.GetTempPath(), $"test_{Guid.NewGuid():N}.json");
        try
        {
    ...
    [Fact]
    public void EmployeeDto_Json_InvalidRows_DetectedCorrectly()
    {
        var path = Path.Combine(Path.GetTempPath(), $"test_{Guid.NewGuid():N}.json");
        try
        {
    ...
    [Fact]
    public void EmployeeDto_Json_FrankRow_HasMultipleErrors()
    {
        var path = Path.Combine(Path.GetTempPath(), $"test_{Guid.NewGuid():N}.json");
        try
        {
            ...
            // Frank is row 6 (1-based index in array)
    ...
    [Fact]
    public void EmployeeDto_Json_FluentValidation_AddsExtraErrors()
    {
        var path = Path.Combine(Path.GetTempPath(), $"test_{Guid.NewGuid():N}.json");
        try
        {
    ...
    [Fact]
    public void EmployeeDto_Json_FullPipeline_GeneratesBothReports()
    {
        var path = Path.Combine(Path.GetTempPath(), $"test_{Guid.NewGuid():N}.json");
