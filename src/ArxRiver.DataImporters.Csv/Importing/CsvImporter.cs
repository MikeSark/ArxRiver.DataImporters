...
/// <summary>
/// Generic CSV importer that reads rows from a CSV file into typed DTOs, validates them, and generates reports.
/// </summary>
public sealed class CsvImporter<T> where T : class, new()
{
    ...
    /// <param name="delimiter">Field delimiter character. Default is comma.</param>
    public CsvImporter(string filePath, bool hasHeaderRow = true, char delimiter = ',')
    {
        _filePath = filePath;
    ...
    /// </summary>
    public CsvImporter<T> ForColumn<TProp>(Expression<Func<T, TProp>> selector,
    ...
    /// <summary>
    /// Reads the CSV file and returns a readonly collection of DTOs.
    /// </summary>
    public ReadOnlyCollection<T> Import()
    {
        ...
        CsvColumnMapping<T> mapping;
        int dataStart;

        if (_hasHeaderRow)
        {
            var headers = CsvParser.ParseLine(lines[0], _delimiter);
            mapping = CsvColumnMapping<T>.Build(headers);
            dataStart = 1;
        }
        else
        {
            mapping = CsvColumnMapping<T>.BuildByNumber();
            dataStart = 0;
        }
        ...
        for (var i = dataStart; i < lines.Length; i++)
        {
            ...
            var fields = CsvParser.ParseLine(line, _delimiter);
            var dto = new T();
